...
static int test_mod_exp_1(int i, BN_CTX *ctx)
{
    ...
    if (!(result = BN_new())
            || !(BN_dec2bn(&base, test->base))
            || !(BN_dec2bn(&exponent, test->exp))
            || !(BN_dec2bn(&modulo, test->mod)))
        goto err;

    if (BN_mod_exp(result, base, exponent, modulo, ctx) != 1)
        goto err;

    if (!(s = BN_bn2dec(result)))
        goto err;

    if (strlen(s) != strlen(test->res) || memcmp(s, test->res, strlen(s)))
        goto err;
...
static int test_mod_exp_consttime_1(int i, BN_CTX *ctx)
{
    const MOD_EXP_TEST *test = &ModExpTests[i];
    int res = 0;
    BIGNUM* result = NULL;
...
int main(int argc, char *argv[])
{
    ...
    {   int i;
        for (i = 0; i < (int)(sizeof(ModExpTests)/sizeof(ModExpTests[0])); i++)
            if (!test_mod_exp_1(i, ctx)
                || !test_mod_exp_consttime_1(i, ctx))
                goto err;
    }
